...
| `ListShippers`    | `pagination` (`Page::parse` folds the request-checksum guard + offset token verification + AIP-158 size policy via `SizeLimits`, then `next_token` mints the follow-on token; read through the generated `PageRequest` impl) + `softdelete` (AIP-164 `show_deleted` filtering) | wired |
