...
| `ListShippers`    | `pagination` (offset page-token codec + request-checksum guard, read through the generated `PageRequest` impl) + `softdelete` (AIP-164 `show_deleted` filtering) | #6, #7, #96, #116 | wired²¹⁰ |
...
Those filterable identifiers are no longer hand-spelled: `Declarations::for_message::<Site>().fields([...])`
derives each one's type from the `Site` descriptor — including the full enum
treatment for `state` (value names + `=`/`!=` overloads) with no caller-side
`Kind::Enum` extraction — so the allow-list can't silently drift from the proto (#127).
...
changes mid-pagination. The `PageRequest` accessors the codec reads through are
**generated** (#116, ADR-0013) — emitted into `freight_service.aip.rs` from each
request's field shape, replacing the hand-written impls that lived in
`service.rs`. `ListShippersRequest` carries only the pagination fields,
