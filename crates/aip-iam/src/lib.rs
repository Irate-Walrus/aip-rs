...
/// (ADR-0011, amending ADR-0010) — and are re-exported here so the public path
/// `aip::iam::proto::…` is unchanged. This is the structural layer the
/// read-modify-write ops (binding add/remove, dedupe, the `etag` cycle) build
/// on; the parse/validate core above stays proto-free, so a default build pulls
/// in no proto runtime.
///
